...
export default function CountryFlagSearch() {
    ...
    <div className="w-full max-w-md">
            ...
            <div className="max-h-80 overflow-y-auto">
              {filteredCountries.map((item, index) => (
                <button
                  key={`${item.code}-${index}`}
                  onClick={() => {
                    setSearchQuery(`${item.country} (${item.code})`);
                    setIsOpen(false);
                  }}
        ...
        {isOpen && searchQuery.trim() && filteredCountries.length === 0 && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-zinc-900 border-2 border-emerald-500/30 rounded-lg p-4">
            <p className="text-zinc-400 text-sm text-center">
              No countries found matching &ldquo;{searchQuery}&ldquo;
            </p>
          </div>
        )}
      </div>
      ...
      {searchQuery && (
        <div className="mt-3 text-xs text-zinc-500">
          <p>
            Found <span className="text-emerald-400 font-semibold">{filteredCountries.length}</span> result{filteredCountries.length !== 1 ? 's' : ''}
          </p>
        </div>
      )}
    </div>
